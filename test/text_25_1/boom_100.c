...
int amount, size_left = INT_MAX, size_right = INT_MIN;
...
int main(void)
{
    ...
    for(int i = 0; i < amount; i++)
    {
        ...
        size_left = min(size_left, x);
        size_right = max(size_right, x);
    }
    ...
    for(int i = 0; i < amount - 1; i++)
    {
        ...
        long long left_result = (long long)(max_y[L][i] - min_y[L][i] + 1) * (monster[i].x - size_left + 1) * max_hp[L][i];
        long long right_result = (long long)(max_y[R][i + 1] - min_y[R][i + 1] + 1) * (size_right - monster[i + 1].x + 1) * max_hp[R][i + 1]; 
