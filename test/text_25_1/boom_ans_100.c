...
int main(void)
{
    ...
    {
        max_y[R][i] = max(max_y[R][i + 1], monster[i + 1].y);
        min_y[R][i] = min(min_y[R][i + 1], monster[i + 1].y);
        max_hp[R][i] = max(max_hp[R][i + 1], monster[i + 1].hp);
    }
    ...
    for(int i = 0; i < amount - 1; i++)
    {
        ...
        long long right_result = (long long)(max_y[R][i] - min_y[R][i] + 1) * (right - monster[i + 1].x + 1) * max_hp[R][i];
